...
module.exports = {
  ...eslint.${framework}${useESLintWithPrettier ? 'Prettier' : ''},
};
`,
        );
      }
...
module.exports = {
  ...stylelint.${css}${useStylelintWithPrettier ? 'Prettier' : ''},
};
`,
        );
      }
...
`,
      ...
      if (config.includes('lint-staged')) {
        ...
        const lintStagedObject: Record<string, string> = {};
        if (config.includes('markdownlint')) {
      ...
      shell.echo('Error:', error?.message ?? error);
      // do something?
    }
  });
